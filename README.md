...
### Кебаб меню - три горизонтальные точки

Вы можете очень быстро добавить рабочий кебаб к себе на страницу, для этого нужно:

1. В html вызвать сниппет `g-kebab` и `g-menu`
2. На ваше потенциальное меню в html добавить атрибут `data-menu`
3. В scss вызвать миксин `kebab`

```
.kebab { @include kebab }
```

4. Зайти в файл js/\_functions.js и раскомментировать строку с подключением js-файла кебаб
5. Настроить стили показа меню под себя с помощью класса `menu--active`
6. Если есть подменю, то присутствует иконка стрелочка, шеврон с атрибутом [data-dropdownicon] (это для js).
